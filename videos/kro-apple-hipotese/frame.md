...
colors:
  ...
  # — Apple-neutral register (the page itself) —
  surface: "#F5F5F7"
  hairline: "#D2D2D7"
  ink-deep: "#000000"
  # — KRO register (system, only where the system acts) —
  kro: "#06B6D4"
  kro-dim: "rgba(6, 182, 212, 0.35)"
  kro-wash: "rgba(6, 182, 212, 0.06)"
  kro-brand: "#7C3AED"
  stage: "#0A0A0C"
...
## Two registers (project-specific doctrine)

This video holds **two visual identities that must never blur into each other**.

**Register A — the page (Apple-neutral).** `bg` pure white, `surface` #F5F5F7,
`text` #1D1D1F, `text-muted` #86868B, `hairline` #D2D2D7. Enormous negative
space. Product is the protagonist. Typography precise, tight tracking, never
decorative. **Zero `kro` accent may appear in this register.** When the frame is
showing the page as itself, the palette is white / black / neutral gray only.

**Register B — the system (KRO).** Enters only when the system acts. `stage`
#0A0A0C is the deep ground the page floats on once it becomes an object in
space. `kro` #06B6D4 is the **single accent**, and it is rationed to exactly six
jobs: selection · test · user flow · metric · winning variant · learning. It is
never used for mood, glow, or decoration. `kro-brand` #7C3AED appears **only**
in the end-card lockup — the brand signature, not the system color.

**The accent is behavior, not decoration.** If cyan appears somewhere the system
is not actively doing one of its six jobs, it is a bug.

**Type ramp is Apple-shaped.** Display sizes carry `-0.02em` tracking and 700
weight; body is 400 at #86868B. Never mix a second display family.

**No shadows, ever.** Depth comes from `perspective` + z-translation + luminance,
never from a drop shadow. No glassmorphism. No blur except as a deliberate
depth-of-field cue on out-of-focus planes — and never on a text HOLD.

## Text HOLD contract (non-negotiable)

Every message is `enter → HOLD → exit`, with HOLD explicitly coded on the
timeline. Enter time never counts as reading time.
...
| 4–7   | 2.5–3.0s     |
| 8–12  | 3.5–4.5s     |
| 13+   | 5.0–6.0s, or split into two compositions |

During HOLD: no blur · no opacity change · no text movement · no tracking
change · no fast zoom · nothing crossing the letters · no strong camera move.
Legibility outranks the duration estimate, always.
