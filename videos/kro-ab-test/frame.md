...
description: >
  ...
  accent, the three-step gray text ladder, Sora (display/numerals/chrome) + Sora (body),
...
colors:
  # The film's base stage-ground, painted by the assembler onto the index #root.
  # It must be the charcoal the frames actually sit on, not the page paper below —
  # a light base flashes through the crossfade seam between two dark frames.
  canvas: "#2B2B2B"
  bg: "#FFFFFF"
  primary: "#06B6D4"
  text: "#2B2B2B"
...
typography:
  # — reading ramp (Sora body + Sora chrome) —
  ...
  # — display / numerical ramp (Sora, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Sora", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Sora", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  ...
  metric-value:{ fontFamily: "Sora", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
...
### KRO role overrides (hand-corrected after the remix — these win over the prose)

The remix put the brand purple into `text-muted` (body copy in purple) and Sora into both
ramps. Both were corrected above. The roles for this project:

- **`primary` cyan `#06B6D4` is the accent** — every numeral, eyebrow, bar fill, live dot, and
  the champion mark. The preset's "single accent" law holds: cyan is the only accent.
- **`brand` purple `#7C3AED` is a GROUND, never a second accent.** It paints the full-bleed
  field on the two brand frames (letreiro + CTA) where white type sits on it. It never
  competes with cyan inside one frame: a purple-ground frame drops cyan to a single word.
- **`ground-dark` charcoal `#2B2B2B`** is the serious ground and the `text` ink on white.
- **Type carries meaning, per the KRO system:** **Manrope** whenever the frame imitates
  product UI (the mock landing pages, the visitor modals, chrome, labels); **Sora 700** when
  the brand speaks (letreiro, CTA, the champion callout). Do not mix without that reason.
- **`sand` `#D8C0A7`** stays unused here — the piece is too short for a breather beat.
- The mock landing pages are **white ground with charcoal ink** so the copy under test reads
  as a real page, not as KRO's own brand surface.
