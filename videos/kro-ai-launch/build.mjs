...
const VP = { x: 100, y: 170, w: LP_W, h: 1010 };
...
const CARD_H = Math.round(WRAP_H * CARD_SCALE_);
const CARD_Y = 466;
const CARD_X = [30, 376, 722];
const CARD_CX = CARD_X.map((x) => x + CARD_W / 2); // 193, 539, 885
const CARD_SCALE = CARD_W / LP_W; // 0.3705

// The winner, enlarged — same crop again, just nearer the camera.
const WIN_W = 470;
const WIN_SCALE = WIN_W / LP_W;
const WIN_H = Math.round(WRAP_H * WIN_SCALE);
const WIN_X = (W - WIN_W) / 2;
const WIN_Y = 258;
const WIN_PANEL_Y = WIN_Y + WIN_H + 22;

const PANEL_Y = CARD_Y + CARD_H + 18; // 858
const PANEL_H = 200;
...
#ground {
  ...
  background:
    radial-gradient(1100px 780px at 50% 22%, rgba(124,58,237,0.10), transparent 62%),
    radial-gradient(900px 700px at 50% 92%, rgba(6,182,212,0.07), transparent 60%),
...
#page-eyebrow {
  ...
  font-size:16px; font-weight:700; letter-spacing:0.16em; color:rgba(255,255,255,0.66);
}
...
#stage-eyebrow {
  position:absolute; left:0; width:${W}px; top:258px; text-align:center;
  font-size:15px; font-weight:800; letter-spacing:0.18em; color:rgba(255,255,255,0.5);
}
#src-pill {
  position:absolute; left:${W / 2 - 92}px; top:302px; width:184px; height:44px; border-radius:100px;
  border:1px solid rgba(255,255,255,0.16); background:rgba(255,255,255,0.05);
  display:flex; align-items:center; justify-content:center; gap:10px;
  font-size:15px; font-weight:700; color:rgba(255,255,255,0.86); letter-spacing:0.02em;
}
#src-pill span { width:8px; height:8px; border-radius:50%; background:${C.cyan}; }
...
.mp {
  ...
  background:rgba(255,255,255,0.045); padding:16px 18px;
}
.mp-head { display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }
.mp-tag { font-size:12px; font-weight:800; letter-spacing:0.12em; color:rgba(255,255,255,0.62); }
.mp-live { width:7px; height:7px; border-radius:50%; background:${C.cyan}; }
.mp-row { display:flex; align-items:baseline; justify-content:space-between; padding:7px 0; border-top:1px solid rgba(255,255,255,0.08); }
.mp-k { font-size:13px; font-weight:600; color:rgba(255,255,255,0.5); }
.mp-v { font-size:19px; font-weight:700; color:${C.white}; font-variant-numeric:tabular-nums; }
.mp-r { font-size:26px; font-weight:800; color:${C.cyan}; font-variant-numeric:tabular-nums; letter-spacing:-0.01em; }
...
#win-badge {
  position:absolute; height:46px; border-radius:100px; padding:0 26px;
...
#win-metric {
  ...
  font-family:'Sora',sans-serif; font-size:52px; font-weight:800; letter-spacing:-0.03em; color:${C.white};
}
#win-metric-k {
  position:absolute; left:0; width:${W}px; text-align:center;
  font-size:16px; font-weight:600; color:rgba(255,255,255,0.55);
}
...
#brand-mark, #value-mark, #cta-mark {
  position:absolute; left:0; width:${W}px; text-align:center;
  font-family:'Sora',sans-serif; font-size:34px; font-weight:800; letter-spacing:0.02em; color:${C.white};
}
#brand-mark i, #value-mark i, #cta-mark i {
  display:inline-block; width:11px; height:11px; border-radius:3px; margin-right:10px;
  background:linear-gradient(135deg,${C.purple},${C.cyan}); vertical-align:middle;
}
#brand-l1, #brand-l2 {
  position:absolute; left:96px; width:${W - 192}px; text-align:center;
  font-family:'Sora',sans-serif; color:${C.white};
}
#brand-l1 { top:566px; font-size:64px; font-weight:800; line-height:1.12; letter-spacing:-0.035em; }
#brand-l2 { top:812px; font-size:27px; font-weight:400; line-height:1.5; letter-spacing:-0.01em; color:rgba(255,255,255,0.62); }

#value-h {
  position:absolute; left:96px; width:${W - 192}px; top:398px; text-align:center;
  font-family:'Sora',sans-serif; font-size:56px; font-weight:800; line-height:1.12; letter-spacing:-0.035em; color:${C.white};
}
#value-s {
  position:absolute; left:96px; width:${W - 192}px; top:632px; text-align:center;
  font-size:25px; font-weight:500; color:rgba(255,255,255,0.6);
}
.flow-node {
  position:absolute; top:812px; height:74px; border-radius:16px;
...
.flow-arrow { position:absolute; top:844px; height:2px; border-radius:2px; background:rgba(255,255,255,0.22); }

#cta-h {
  position:absolute; left:96px; width:${W - 192}px; top:476px; text-align:center;
  font-family:'Sora',sans-serif; font-size:58px; font-weight:800; line-height:1.1; letter-spacing:-0.035em; color:${C.white};
}
#cta-btn {
  position:absolute; left:${(W - 460) / 2}px; top:726px; width:460px; height:92px; border-radius:100px;
  background:linear-gradient(135deg,${C.purple},${C.cyan});
  display:flex; align-items:center; justify-content:center;
  font-family:'Sora',sans-serif; font-size:27px; font-weight:800; color:${C.white}; letter-spacing:-0.01em;
}
#cta-micro {
  position:absolute; left:0; width:${W}px; top:848px; text-align:center;
  font-size:19px; font-weight:600; color:rgba(255,255,255,0.5);
}
#cta-url {
  position:absolute; left:0; width:${W}px; top:1188px; text-align:center;
  font-size:20px; font-weight:700; letter-spacing:0.1em; color:rgba(255,255,255,0.42);
}
`;
...
const script = `
      ...
      tl.fromTo("#vp", { opacity: 0, y: 54, scale: 0.965 },
        { opacity: 1, y: 0, scale: 1, duration: 0.72, ease: "power3.out" }, 0.05);
      ...
      gsap.set(["#cw-A", "#cw-B", "#cw-C"], { x: HERO_LEFT, y: HERO_TOP, scale: 1 });
      ...
      gsap.set("#win-badge", { x: ${(W - 236) / 2}, y: ${WIN_Y - 64}, width: 236, scale: 0.8, transformOrigin: "50% 50%" });
      gsap.set("#win-metric", { y: ${WIN_PANEL_Y + PANEL_H + 24} });
      gsap.set("#win-metric-k", { y: ${WIN_PANEL_Y + PANEL_H + 96} });
      ...
      tl.to("#cw-A", { scale: 1.05, x: HERO_LEFT - 22, y: HERO_TOP - 16, duration: 0.6, ease: "power2.inOut" }, 4.45);
      ...
      K.forEach(function (k, ci) {
        for (var i = 0; i < 11; i++) {
          ...
          gsap.set(el, { x: ${W / 2} - 3, y: 342 });
          tl.fromTo(el, { opacity: 0, x: ${W / 2} - 3, y: 342 },
      ...
      gsap.set("#value-mark", { y: 258 });
      gsap.set(["#value-mark", "#value-h", "#value-s"], { opacity: 0 });
      var FW = 228, FGAP = 26, FTOT = FW * 4 + FGAP * 3, FX0 = (${W} - FTOT) / 2;
      ...
      tl.fromTo("#value-mark", { y: 272 }, { opacity: 1, y: 258, duration: 0.5, ease: "power3.out" }, 23.85);
      ...
      tl.fromTo("#cta-mark", { y: 344 }, { opacity: 1, y: 330, duration: 0.5, ease: "power3.out" }, 26.65);
